...
DEMO_ROOT = REPO_ROOT / "demo-xuanhuan-short"
...
class DemoXuanhuanShortSampleSmokeTest(unittest.TestCase):
    def setUp(self) -> None:
        self.temp_dir = Path(tempfile.mkdtemp(prefix="story-harness-demo-xuanhuan-short-"))
        self.project_root = self.temp_dir / "demo-xuanhuan-short"
    ...
    def test_demo_xuanhuan_short_sample_supports_xuanhuan_review_workflow(self) -> None:
        ...
        for chapter_id, scene_count in scene_counts.items():
            ...
            self.assertEqual(review["weightedScores"]["profile"]["subGenre"], "xuanhuan")
        ...
        self.assertIn("## 火种入骨", exported_text)
        self.assertIn("## 天门一炷香", exported_text)
